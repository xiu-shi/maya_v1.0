...
export function detectPromptInjection(input) {
  ...
  const suspiciousPatterns = [
    // Pattern: "ignore all previous instructions" or "ignore previous instructions"
    // Handles: "ignore all previous instructions", "ignore previous instructions", "ignore all instructions"
    /ignore\s+(all\s+)?(previous|above)\s+(instructions|prompts|rules)/i,
    /ignore\s+all\s+(instructions|prompts|rules)/i,
    // Pattern: "ignore previous instructions" (without "all")
    /ignore\s+(previous|above)\s+(instructions|prompts|rules)/i,
