...
function validateToken(token) {
  ...
  if (
    ...
    (token === "test-token" || token === "test-token-for-testing")
  ) {
    return token;
  }
  ...
  if (!token) {
    ...
    if (process.env.NODE_ENV === "production") {
      console.warn(
        "⚠️  Warning: AI_BUILDER_TOKEN is not set. Service may not function correctly.",
      );
  ...
  if (!token.startsWith("sk_")) {
    ...
    throw new Error(
      'AI_BUILDER_TOKEN format is invalid - must start with "sk_"',
    );
  }
  ...
  if (token.length < 20) {
    console.error(
      "❌ Error: AI_BUILDER_TOKEN appears to be invalid (too short)",
    );
  ...
  if (placeholderKeys.includes(token)) {
    ...
    console.error(
      "   Please replace with your actual API key from https://space.ai-builders.com",
    );
    throw new Error(
      "AI_BUILDER_TOKEN is a placeholder - please use your actual key",
    );
  }
...
if (config.port !== 0 && (config.port < 1 || config.port > 65535)) {
  throw new Error(
    `Invalid PORT: ${config.port}. Must be between 1 and 65535 (or 0 in test mode)`,
  );
}

if (config.maxMessageLength < 1 || config.maxMessageLength > 10000) {
  throw new Error(
    `Invalid MAX_MESSAGE_LENGTH: ${config.maxMessageLength}. Must be between 1 and 10000`,
  );
}
