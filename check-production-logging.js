...
async function checkProductionLogging() {
  ...
  try {
    ...
    const logsResponse = await fetch(
      `${PRODUCTION_URL}/api/admin/chat-logs?startDate=2026-01-01&endDate=2026-01-28`,
    );
    if (logsResponse.ok) {
      ...
      console.log(
        `   Date range: ${logsData.startDate} to ${logsData.endDate}`,
      );
      console.log("");
    ...
    const statsResponse = await fetch(
      `${PRODUCTION_URL}/api/admin/chat-logs/stats`,
    );
    if (statsResponse.ok) {
      ...
      if (statsData.success) {
        ...
        if (stats.totalFiles > 0 && stats.files) {
          ...
          stats.files.slice(0, 5).forEach((file) => {
            console.log(
              `      - ${file.date}: ${file.messages} messages, ${file.conversations} conversations`,
            );
          });
          console.log("");
        }
    ...
    if (testChatResponse.ok) {
      ...
      const newLogsResponse = await fetch(
        `${PRODUCTION_URL}/api/admin/chat-logs?startDate=${new Date().toISOString().split("T")[0]}&endDate=${new Date().toISOString().split("T")[0]}`,
      );
      if (newLogsResponse.ok) {
        const newLogsData = await newLogsResponse.json();
        const testLogs = Array.isArray(newLogsData.logs)
          ? newLogsData.logs
          : [];
        const testMessageFound = testLogs.some(
          (log) =>
            log.userMessage &&
            log.userMessage.includes("Test message for logging verification"),
        );
    ...
    console.log(
      "   2. Verify data/chat-logs/ directory exists and is writable",
    );
